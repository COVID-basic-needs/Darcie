...
app.post('/event', (req, res) => { // whenever something happends on the Nexmo side of the call it uses this to update us.
  // TODO: add processing DTMF input sent from user after websocket sends 'input' action
  // include:
  // - session UUID
  // - caller phone number
  // - DTMF digits pressed
  // Send all of the above to the watson assistant

  const docRef = db.collection('calls').doc(req.body.conversation_uuid);
...
app.ws('/socket', (ws, req) => { // Nexmo Websocket Handler.
  ...
  let tik = 0; // notes in the firestore DB the order of the chunks of the call (who said what when)
  ...
  const recognizeStream = gSTTclient
    ...
    .on('data', data => { // data is text returned from google
      const userResponse = data.results[0].alternatives[0].transcript;
